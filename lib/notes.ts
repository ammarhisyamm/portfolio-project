...
export const NOTE_COLORS = ["cream", "yellow", "pink", "blue", "sage", "lavender"] as const;
...
export async function getNotes(): Promise<VisitorNote[]> {
  ...
  const { data, error } = await supabase
    .from("visitor_notes")
    .select("id, message, name, color, created_at")
...
export async function insertNote(input: {
  ...
  const { data, error } = await supabase
    .from("visitor_notes")
    .insert({ message: input.message, name: input.name, color: input.color })
    .select("id, message, name, color, created_at")
    .single();
